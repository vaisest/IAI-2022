...
class State:
    ...
    def heuristic(self):
        ...
        # Implement me
        goal = State.goal
        return dist([self.stop["x"], self.stop["y"]], [goal["x"], goal["y"]]) / MAX_TRAM_SPEED
...
class CityMap:
    ...
    def search(self, start, goal, time_of_beginning):
        ...
        # this thing is for threading and i dont like it
        # to_visit = Q.PriorityQueue()
        to_visit = []
        heapq.heappush(to_visit, State(start, time_of_beginning))

        while to_visit:
            node = heapq.heappop(to_visit)
            node_code = node.get_stop_code()

            if node_code == State.goal["code"]:
                return node

            for neighbor_code in self.get_neighbors(node_code).keys():
                travel_time = self.fastest_transition(
                    node_code, neighbor_code, node.current_time
                )

                neighbor_node = self.get_stop(neighbor_code)
                heapq.heappush(
                    to_visit,
                    State(neighbor_node, time=node.current_time + travel_time, previous=node),
                )
