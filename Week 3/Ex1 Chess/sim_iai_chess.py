...
# simulate many below
def percentage_count(arr, item):
    return arr.count(item) / len(arr)


results = []
while True:
    results.append(play_game(depth=3))
    print(
        f"Wins: {percentage_count(results, True):.2f} D+L: {1-percentage_count(results, True):.2f}Losses: {percentage_count(results, False):.2f} Draws: {percentage_count(results, None):.2f} n: {len(results)} counts: {collections.Counter(results)}"
    )
